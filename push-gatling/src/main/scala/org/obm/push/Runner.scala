...
class Runner(simulation: Simulation, simulationId: String, description: String) extends AkkaDefaults with Logging {

	def run: (String, Simulation) = {

		try {
			val simulationName = simulation.getClass().getName()
			println(s"Simulation ${simulationName} started...")

			val runMessage = RunMessage(now, simulationId, description)

			val scenarios = simulation.scenarios

			require(!scenarios.isEmpty, s"${simulationName} returned an empty scenario list. Did you forget to migrate your Simulations?")
			...
			terminatorLatch.await(gatlingConfiguration.core.timeOut.simulation, SECONDS)
