== Providing access to the Opush server
...
When you try to add an ActiveSync account on your smartphone, you have set the
user email address. +
...
Some phones prefix the domain by +autodiscover.+. So you must add the following 
...
There are several things you can configure about Opush mail handling. Below is
the default +/etc/opush/mail_conf.ini+ file:
[source]
----
...
Usually, it's used for recipient auto-completion when the user writes an email.
...
<<_providing_access_to_opush_server>> in order to have Autodiscover working

== Administration operation

=== Resetting a device

For some reasons, users may need to reset their smartphones.
This can be done by two different ways in OBM:

* by OBM-UI: the OBM administrator has to delete the mobile device in the
	dedicated user page
* by the smartphone itself: the smartphone user has to modify the filter type
	(amount to synchronize) in the ActiveSync account settings

Here is a list of smartphones which are known to be compatible with this
operation:

|=======================================================================
|*Device*				| *By OBM-UI*	|*By smartphone*|
