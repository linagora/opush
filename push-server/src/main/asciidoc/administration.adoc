...
For some reasons, users may need to reset their devices.
...
* by the device itself: the device user has to modify the filter type
	(amount to synchronize) in the ActiveSync account settings
...
spushnik documentation for more information.

== Provide Access to the Opush Server
...
WARNING: be careful, +autodiscover+ is supposed to be case-insensitive
...
When you try to add an ActiveSync account on your device, you have set the
user email address. +
...
by the http://tools.ietf.org/html/rfc3501[IMAP RFC 3501], section 5.1.
...
In Opush, LDAP is only used for user lookups, depending on the clients requests.

Usually, it is used for recipient auto-completion when the user writes an email.

[WARNING]
====
LDAP is neither used directly by Opush to populate the client address books nor
to authenticate anybody. These two features are delegated to obm-sync.
====
...
The Autodiscover feature allows devices to find a working configuration by 
themselves based on a user email address.

This feature requires a secure communication channel between the device and the
server.

Even if it could work without any certificate depending on the device used,
most devices require a valid SSL certificate.
...
If you have a self-signed certificate, you need to import this certificate on
each device. Please refer to your device documentation.
...
<<_provide_access_to_the_opush_server>> in order to have Autodiscover working
...
Opush, like all *Java* components of *OBM*, uses a logger API to generate information
...
* +REQUEST:1147+ means that the whole request execution took *1147ms* +
  ** +EMAIL:1095+: in *REQUEST*, *1095ms* has been spent in the *EMAIL* category +
    *** +CASSANDRA:16+: in *EMAIL*, *16ms* has been spent in the *CASSANDRA* category +
    *** +SQL:3+: in *EMAIL*, *3ms* has been spent in the *SQL* category +
    *** +OTHER:1076+: in *EMAIL*, *1076ms* has been spent in treatments belonging to undefined category  +
  ** +CASSANDRA:14+: in *REQUEST*, *14ms* has been spent in the *CASSANDRA* category +
  ** +SQL:11+: in *REQUEST*, *11ms* has been spent in the *SQL* category +
  ** +OTHER:27+: in *REQUEST*, *27ms* has been spent in treatments belonging to to undefined category  +

Interpretation of these values can be useful to understand performance issues of your Opush server. +
What is interesting in this example is +OTHER:1076+, it means that 1076ms has been spent in unnamed code category of *EMAIL*. +
It might be the imap server which is really long to treat Opush requests.

There are few rules applied to these logs that could be great to know:
...
* Children of a category are ordered by descending duration time except the +OTHER+ entry that is always the last
