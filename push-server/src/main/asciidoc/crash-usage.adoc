...
=== Granting Rights to the Admin Profile

As Opush may manage multiple domains, +admin+ user's profile should have all rights on +domains+.

To give all +domains+ rights to a profile, authenticate with the OBM web interface as an admin for the global domain (like +admin0+), click on +Administration+, +User profiles+ then update the +admin+ profile.
...
This command installs a Cassandra schema on a new Cassandra server. It is intended to be run only after the first Opush installation. You won't be able to use Opush until this schema is installed
